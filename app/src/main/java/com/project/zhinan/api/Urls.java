package com.project.zhinan.api;

/**
 * Created by ymh on 2016/4/11.
 */
public class Urls {
    //http://123.206.84.242:2888/getAds?tag=fangchan&page=1
    public static final String BaseUrl = "http://120.27.41.245:3000/";
    public static final String TagUrl = "http://123.206.84.242:2888/getAds?tag=";
    public static final String PageNum = "&page=1";
    public static final String BaoxiangUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String CanyinUrl = TagUrl + "canyin" + PageNum;
    public static final String FangchanUrl = TagUrl + "fangchan" + PageNum;
    public static final String FuwuUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String GongyiUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String HongbaoUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String HuodongUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String JiancaiUrl = TagUrl + "jiancai" + PageNum;
    public static final String JinrongUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String Jiaoyu = TagUrl + "jiaoyu" + PageNum;
    public static final String LvyouUrl = TagUrl + "lvyou" + PageNum;
    public static final String MeizhuangUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String QianggouUrl = "http://119.29.191.229:2888/com/try";
    public static final String QicheUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String ShangquanUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String TongxunUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String XiuxianUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String YiyaoUrl = TagUrl + "yiyao" + PageNum;
    public static final String ZhaopinUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String ZhaoshangUrl = TagUrl + "jiaoyu" + PageNum;
    public static final String DetailUrl = "http://120.27.41.245:2888/tryhtml.html";
    public static final String Collection_Url = "http://123.206.84.242:2888/collection_c";

}
